...
{
    if (*(first_page_address + PAGE_SIZE_BYTES / 4) == 0xFFFFFFFF)
        return (uint32_t)first_page_address;
    else
        return (uint32_t)first_page_address + PAGE_SIZE_BYTES / 4;
}
...
void write_hsv_actual_values(hsv_t *hsv_vals)
{
    ...
    active_page_start_addr = get_active_page_start_addr((uint32_t *)APP_DATA_START_ADDR);
...
void read_hsv_actual_values(hsv_t *hsv_vals)
{
    ...
    {
        NRF_LOG_INFO("At address %d h=%d s=%d v=%d", addr, *(uint16_t *)addr, *(uint8_t *) (addr + 2), *(uint8_t *) (addr + 3));
    }
...
void read_hsv_values_for_log(hsv_t *hsv_vals)
{
    ...
    NRF_LOG_INFO("-----Reading %d bytes finished-----", current_read_address - active_page_start_addr + 4);
}

uint8_t hsv_crc_calc(hsv_t *hsv_vals)
{
